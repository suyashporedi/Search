...
public class feedbackTermsTFIDF {
	...
	public static void main (String args[]) throws ParseException, IOException {
		...
		while (topIterator.hasNext()) {
			...
			HashMap<String,Double> titleVector=FeatureVector(topicTitle);
			HashMap<String,Double> relevantVector = FeatureVector(relevantList);
			HashMap<String, Double> irrelVector = FeatureVector(irrelevantList);
			titleVector = calculateTFIDFForVector(reader, titleVector);
			
			relevantVector = calculateTFIDFForVector(reader, relevantVector);
			irrelVector = calculateTFIDFForVector(reader, irrelVector);
...
//							
				...
				String path = "./Beta"+beta+"Gama"+gamma;
	...
	public static void executeResult(LinkedHashMap<Integer,String> queries,IndexReader reader,
		...
		for (Integer qid: queries.keySet()) {
			...
			String fileName = "./TFIDF/" + FileName + ".txt";
...
// Calculate TFIDF for Vectors to remove noisy terms
public static HashMap<String, Double> calculateTFIDFForVector (IndexReader reader, HashMap<String, Double> vector) throws IOException {
		int numDocs = reader.maxDoc()+1;
		HashMap<String, Double> newvector = new HashMap<String, Double>();
		for (String term : vector.keySet()) {
			int dfTerm = reader.docFreq(new Term("TEXT", term))+1;
			newvector.put(term,vector.get(term)*Math.log10(1+(numDocs/dfTerm)));
		}
		return(newvector);
	}
